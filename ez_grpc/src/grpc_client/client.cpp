...
{
	auto connection = CreateConnectionInternal(ip, port);
...
std::shared_ptr<ClientConnectInterface> Client::CreateLongConnection()
{
...
std::shared_ptr<ClientConnectInterface> Client::CreateConnectionInternal(const std::string& ip, int port)
{
	auto connection = std::make_shared<ClientConnectInternal>(ip, port);
	if (connection == nullptr)
	{
		return nullptr;
	}
